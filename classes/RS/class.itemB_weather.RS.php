...
class item_based_filteredBy_weather
{
  public $pdo;
  public $item_based;
  public $datasetClass;
  ...
  {
    $this->pdo = $pdo;
    $this->item_based = new item_based($pdo);
    $this->datasetClass = new dataset($pdo);
  }



  //******************************* item based_RS time filtered recommendation **************************************

  public function recommend($user_id, $weather, $max_reco)
  {
    //Get the dataset filtered by time.
    $dataset = $this->datasetClass->get_Dataset_filteredBy_weather($weather);

    //Get the predictions of the current user (not-rated) POIs ratings.
    $predictions_list = $this->item_based->predict($user_id, $dataset);

    //Get the user ratings average.
    $user_average = $this->item_based->user_ratings_average($user_id, $dataset);
